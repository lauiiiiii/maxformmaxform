      ...
      <div class="container hero-vert-inner">
        <div class="hero-text center-block">
          ...
          <p class="subtitle large">
            告别云端风险，拥抱内网安心。<br />从数据到决策，一站式闭环解决方案，为组织协同与数据洞察保驾护航
          </p>
          <div class="hero-cta center">
            <el-button type="primary" size="large" @click="goEntry">{{
              isAuthed ? '进入工作台' : '立即体验'
            }}</el-button>
          ...
          <ul class="hero-bullets center">
            <li
              ...
              @click="clickBullet(b)"
              class="clickable"
              :class="{ active: activeBullet === b }"
              :aria-current="activeBullet === b ? 'true' : undefined"
            >
              {{ b }}
            </li>
          </ul>
        ...
        <div class="hero-secure-wrap" role="group" aria-label="政企级安全与合规概览">
          <div class="secure-panel anim-safe">
            <div class="secure-head">
              <svg class="icon-shield" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  fill="currentColor"
                  d="M12 2.5 4.5 5v6.9c0 5.07 3.36 9.72 7.5 10.6 4.14-.88 7.5-5.53 7.5-10.6V5L12 2.5Zm0 2.05 5.5 1.87v5.48c0 4.13-2.7 7.96-5.5 8.77-2.8-.81-5.5-4.64-5.5-8.77V6.42L12 4.55Zm-1 5.95 4.24-4.24 1.06 1.06L11 12.66l-3.3-3.3 1.06-1.06L11 10.5Z"
                />
              </svg>
              ...
              <div class="secure-card grad-mint" aria-label="合规支撑">
                <div class="row-top">
                  ...
                  <span class="mini-stats" aria-label="合规推进进度">
                    <em>ISO</em><span class="bar"><i style="--p: 0.65"></i></span> <em>等保</em
                    ><span class="bar"><i style="--p: 0.4"></i></span>
                  </span>
                </div>
      ...
      <div class="container">
        ...
        <div class="deployment-card anim-fade">
          <div class="deploy-content">
            <h3>统一企业版</h3>
            <p class="muted">
              所有能力一次性开放：题型全集 · 权限/审批 · 审计日志 · 安全加固 · 多租户隔离 · 插件扩展
              · 数据分析 · 私有化交付。
            </p>
        ...
        <div class="arch-wrap anim-fade">
          <div class="arch-diagram" aria-label="系统架构拓扑示意">
            <svg viewBox="0 0 600 280" class="arch-svg">
              ...
              <text x="300" y="56" font-size="18" text-anchor="middle" fill="#fff">
                前端 (Web / 移动端 H5)
              </text>
              <rect x="20" y="100" width="560" height="60" rx="12" fill="#fff" stroke="#dbe3ef" />
              <text x="300" y="136" font-size="16" text-anchor="middle" fill="#333">
                API Gateway / 认证 / 限流 / 日志
              </text>
              <rect x="20" y="180" width="180" height="80" rx="12" fill="#fff" stroke="#dbe3ef" />
              <text x="110" y="216" font-size="14" text-anchor="middle" fill="#333">
                Survey 服务
              </text>
              <rect x="210" y="180" width="180" height="80" rx="12" fill="#fff" stroke="#dbe3ef" />
              <text x="300" y="216" font-size="14" text-anchor="middle" fill="#333">
                分析/统计 服务
              </text>
              <rect x="400" y="180" width="180" height="80" rx="12" fill="#fff" stroke="#dbe3ef" />
              <text x="490" y="216" font-size="14" text-anchor="middle" fill="#333">
                插件/扩展 服务
              </text>
            </svg>
          </div>
    ...
    <section id="metrics" class="section alt metrics-modern">
      <div class="container metrics-flex">
        <div class="metrics-text anim-fade">
          ...
          <p class="section-desc align-left small-desc">
            核心能力与效率指标为企业级稳定运行提供信心。
          </p>
...
const goEntry = () => {
  if (isAuthed.value) return router.push('/user-dashboard')
  router.push('/login')
}
...
watch(isDark, (data) => {
  darkStore.setDark(data)
})

// 滚动时根据当前视口更新激活 bullet
const updateActiveBullet = () => {
  const sections = Object.entries(bulletMap).map(([k, sel]) => ({
    k,
...
const securityPoints = [
  {
    icon: '🔐',
    title: '加密体系',
    desc: '传输 HTTPS/TLS1.2+，存储敏感字段可选字段级加密。',
    extra: ['可插拔 KMS', '加盐哈希', '密钥轮换策略'],
  },
  {
    icon: '🧾',
    title: '审计追踪',
    desc: '操作/导出/权限变更留痕，支持过滤检索与合规留存。',
    extra: ['导出签名', '时间线追踪'],
  },
  {
    icon: '🧱',
    title: '租户隔离',
    desc: '按 enterpriseId 严格逻辑隔离，预留物理隔离选项。',
    extra: ['资源限额', '名称空间策略'],
  },
  {
    icon: '🛡️',
    title: '访问控制',
    desc: '最小权限 RBAC + 细粒度数据访问控制。',
    extra: ['审批联动', '敏感字段脱敏'],
  },
  {
    icon: '📦',
    title: '备份恢复',
    desc: '多层快照 + 增量备份，支持演练与 RPO/RTO 目标。',
    extra: ['异地副本', '自动校验'],
  },
  {
    icon: '📜',
    title: '合规支持',
    desc: '支持等保/ISO 基线与内部安全审计抽查。',
    extra: ['漏洞响应流程', '安全白皮书(占位)'],
  },
]
...
onMounted(() => {
  ...
  document
    .querySelectorAll('.anim-fade, .step, .scenario, .tech-pill, .case-logo, .metric-card')
    .forEach((el) => obs.observe(el))

  const obsMetrics = new IntersectionObserver(
    (entries) => {
      entries.forEach((e) => {
        if (e.isIntersecting) {
  ...
  document.querySelectorAll('.metric-card').forEach((el, i) => {
    el.setAttribute('data-midx', String(i))
    obsMetrics.observe(el)
  })
...
const faqs = [
  {
    q: '数据是否属于我们企业？',
    a: '是。平台仅作为处理与呈现载体，数据所有权与处置权完全归属企业，可随时 <strong>导出/脱敏/销毁</strong>。',
  },
  {
    q: '是否支持私有化与离线环境部署？',
    a: '支持。在内网可通过 Docker / K8s 交付，支持封闭网络下镜像与离线依赖清单。',
  },
  {
    q: '安全与审计如何保障？',
    a: '操作审计、权限变更、导出行为全留痕；支持分级脱敏与最小权限模型。',
  },
  { q: '能否与现有统一身份系统集成？', a: '可扩展 LDAP / OIDC / SAML，提供回调与角色映射。' },
  {
    q: '如何防止重复或批量刷答？',
    a: '指纹/行为节奏/答题耗时阈值 + 题目随机化 + IP / UA 组合策略。',
  },
  {
    q: '问卷更新后历史统计会变吗？',
    a: '系统保留版本快照并按发布版本聚合，历史口径不被新题目影响。',
  },
]
...
interface FooterItem {
  label: string
  href: string
  badge?: { text: string; type: 'hot' | 'soon' }
}
...
:root {
  --c-bg: #0f141c;
  --c-grad: linear-gradient(120deg, #4f8bff, #6f62ff 40%, #2dd9c5);
  --c-text: #1f2d3d;
  --c-alt: #f6f9fc;
}
.home-wrapper {
  font-family:
    ...
    sans-serif;
  color: var(--c-text);
  background: linear-gradient(180deg, var(--color-bg-base) 0%, #f4f8ff 60%, #eef4ff 100%);
}
.container {
  max-width: 1180px;
  margin: 0 auto;
...
.brand-logo {
  height: var(--brand-logo-size, 90px);
  display: block;
  object-fit: contain;
}
.links {
...
.hero::before {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 20% 20%, #d2e6ff, transparent 60%),
    radial-gradient(circle at 80% 40%, #e4d9ff, transparent 55%);
  opacity: 0.8;
  pointer-events: none;
}
.hero-vert-inner,
...
.secure-card.grad-blue {
  background: linear-gradient(110deg, #5d6bff, #4866ff 48%, #cddaf6);
}
.secure-card.grad-mint {
  background: linear-gradient(110deg, #27d5c7, #1cb0e6 55%, #cfe8f5);
}
.secure-card.pale {
  background: linear-gradient(110deg, #f3f6fa, #ffffff);
...
.secure-card.pale .s-desc {
  color: #5b6775;
}
.secure-card .row-meta {
  margin-top: auto;
}
...
.grad {
  background: var(--c-grad, linear-gradient(90deg, #2d5bff, #639bff));
  -webkit-background-clip: text;
  color: transparent;
}
.no-bgclip .grad {
...
.hl {
  ...
  font-weight: 600;
  color: #246bff;
  background: linear-gradient(90deg, #4f8bff, #7faeff);
  -webkit-background-clip: text;
  color: transparent;
}
.hl:after {
  ...
  bottom: -2px;
  height: 6px;
  background: linear-gradient(90deg, rgba(79, 139, 255, 0.25), rgba(127, 174, 255, 0.05));
  border-radius: 3px;
  pointer-events: none;
}
.hero-cta {
...
.hero-bullets li.clickable {
  cursor: pointer;
  transition: 0.25s;
}
.hero-bullets li.clickable:hover {
  background: #4f8bff;
  color: #fff;
  border-color: #4f8bff;
  box-shadow: 0 4px 14px -4px rgba(79, 139, 255, 0.45);
}
.hero-bullets li.active {
  background: #246bff;
  color: #fff;
  border-color: #246bff;
  box-shadow: 0 4px 18px -4px rgba(36, 107, 255, 0.55);
}
.dark .hero-bullets li {
  background: #1e2733;
  border-color: #2d3744;
}
.dark .hero-bullets li.active {
  background: #367dff;
  border-color: #367dff;
}
.hero-art {
...
.secure-panel {
  ...
  box-shadow: 0 20px 52px -18px rgba(54, 83, 148, 0.35);
  position: relative;
...
.secure-panel::before {
  ...
  background:
    radial-gradient(circle at 78% 12%, rgba(79, 139, 255, 0.2), transparent 62%),
    radial-gradient(circle at 8% 92%, rgba(45, 217, 197, 0.18), transparent 65%);
  z-index: -1;
  filter: blur(6px);
}
.secure-head {
...
.secure-row.has-accent::before {
  ...
  width: 4px;
  border-radius: 4px;
  background: linear-gradient(180deg, #4f8bff, #6f62ff);
  opacity: 0.95;
}
.secure-row.grad-blue {
  background: linear-gradient(110deg, #5d6bff, #4866ff 45%, #cddaf6);
}
.secure-row.grad-mint {
  background: linear-gradient(110deg, #27d5c7, #1cb0e6 50%, #cfe8f5);
}
.secure-row.grad-mint.has-accent::before {
  background: linear-gradient(180deg, #27d5c7, #1cb0e6);
}
.secure-row.pale {
  background: linear-gradient(110deg, #f3f6fa, #ffffff);
  border: 1px solid #dfe6ef;
}
.secure-row.pale.has-accent::before {
  background: linear-gradient(180deg, #cfd7e2, #b9c4d3);
  opacity: 0.8;
}
.row-top {
  display: flex;
...
.bar i {
  position: absolute;
  inset: 0;
  background: linear-gradient(90deg, #fff, #e5f5ff);
...
.dots .dot.on {
  background: linear-gradient(135deg, #4f8bff, #6f62ff);
...
.anim-safe {
  ...
  transition: 0.7s cubic-bezier(0.16, 0.8, 0.34, 1);
}
.anim-safe.in-view {
  opacity: 1;
...
.feature-card:hover {
  ...
  box-shadow: 0 12px 28px -8px rgba(79, 139, 255, 0.25);
}
...
.scenario:hover {
  border-color: #4f8bff;
  box-shadow: 0 6px 18px -4px rgba(79, 139, 255, 0.25);
}
.scenario h4 {
  margin: 0 0 8px;
  font-size: 16px;
...
.deployment-card {
  ...
  position: relative;
  overflow: hidden;
  box-shadow: 0 18px 48px -14px rgba(54, 83, 148, 0.25);
}
.deployment-card::before {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 85% 15%, rgba(79, 139, 255, 0.18), transparent 60%),
    radial-gradient(circle at 10% 80%, rgba(45, 217, 197, 0.18), transparent 55%);
  pointer-events: none;
}
.deploy-content {
  position: relative;
...
.deploy-points li::before {
  ...
  height: 8px;
  border-radius: 50%;
  background: linear-gradient(135deg, var(--color-brand-primary), var(--color-brand-mint));
}
.deploy-cta {
  display: flex;
  gap: 16px;
...
.security-item:hover {
  box-shadow: 0 14px 42px -12px rgba(79, 139, 255, 0.35);
  transform: translateY(-6px);
}
.dark .security-item {
  background: #18212b;
...
.assurance-banner {
  margin-top: 40px;
  background: linear-gradient(110deg, #4f8bff, #6f62ff);
  color: #fff;
  padding: 18px 28px;
  border-radius: 16px;
  font-size: 14px;
  box-shadow: 0 10px 30px -12px rgba(79, 139, 255, 0.5);
}
...
.gov-step:hover {
  transform: translateY(-6px);
  box-shadow: 0 14px 36px -12px rgba(79, 139, 255, 0.3);
}
.gov-index {
  position: absolute;
  top: -14px;
...
.arch-diagram {
  ...
  border-radius: 22px;
  padding: 18px;
  box-shadow: 0 10px 32px -14px rgba(79, 139, 255, 0.25);
}
.dark .arch-diagram {
  background: #18212b;
}
...
.cta-modern {
  position: relative;
  background: linear-gradient(135deg, #4f8bff 0%, #6f62ff 55%, #2dd9c5);
  color: #fff;
  padding: 90px 0 40px;
  overflow: hidden;
}
...
.cta-bg-shape {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 85% 20%, rgba(255, 255, 255, 0.35), transparent 60%),
    radial-gradient(circle at 15% 80%, rgba(255, 255, 255, 0.25), transparent 55%);
  opacity: 0.6;
}
.wave-sep {
  height: 50px;
...
.metric-card {
  ...
  text-align: left;
  box-shadow: 0 8px 28px -10px rgba(54, 83, 148, 0.25);
  overflow: hidden;
}
.dark .metric-card {
...
.metric-card::after {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(circle at 85% 20%, rgba(79, 139, 255, 0.35), transparent 55%);
  opacity: 0.55;
}
...
.modern-banner {
  ...
  max-width: 1000px;
  border-radius: 18px;
  background: linear-gradient(120deg, #4f8bff, #6f62ff 55%, #2dd9c5);
  color: #fff;
  padding: 18px 26px;
  font-size: 14px;
  box-shadow: 0 10px 32px -14px rgba(79, 139, 255, 0.45);
}
@media (max-width: 900px) {
  .metrics-flex {
    flex-direction: column;
...
.logo-box {
  ...
  font-size: 16px;
  letter-spacing: 1px;
  box-shadow: 0 4px 14px -6px rgba(54, 83, 148, 0.18);
  transition: var(--transition-base);
  position: relative;
  overflow: hidden;
}
.logo-box::after {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(120deg, rgba(79, 139, 255, 0), rgba(79, 139, 255, 0.15));
  opacity: 0;
  transition: var(--transition-base);
}
.case-logo:hover .logo-box {
  transform: translateY(-4px);
  box-shadow: 0 10px 28px -10px rgba(79, 139, 255, 0.35);
}
.case-logo:hover .logo-box::after {
  opacity: 1;
}
.logo-name {
  font-size: 11px;
